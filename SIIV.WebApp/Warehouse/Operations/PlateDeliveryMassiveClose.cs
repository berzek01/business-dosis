...
// Type: SIIV.WebApp.Warehouse.Operations.PlateDeliveryMassiveClose
...
{
  public class PlateDeliveryMassiveClose : Page
  {
    ...
    protected void rblPersonTypeCollect_SelectedIndexChanged(object sender, EventArgs e)
    {
      try
      {
        ...
        string pstrUseCorrespondence = this.hdUseCorrespondence.Value;
        switch (result2)
        {
          case 1:
            this.trNotaria.Style.Add("display", "");
            this.trPartida.Style.Add("display", "none");
            this.LoadRequisiteToDeliver(result1, result2, 2, 1, pstrUseCorrespondence, 0);
            break;
          case 2:
            if (this.rblPersonTypeCollect.SelectedValue == "2")
            {
              ...
              this.LoadRequisiteToDeliver(result1, result2, 2, 1, pstrUseCorrespondence, 2);
            }
            else
            {
              this.trNotaria.Style.Add("display", "");
              this.trPartida.Style.Add("display", "");
              this.LoadRequisiteToDeliver(result1, result2, 2, 1, pstrUseCorrespondence, 1);
            }
            break;
        }
      }
      ...
      try
      {
        if (this.txtName.Text != "")
        {
          if (this.txtName.Text.Trim().Length <= 3)
            throw new HandledException(1, Constants.REQUIREMENT_ADVERTENCIA_Ingrese_Nombre_Razon);
        }
        else
        {
          if (this.wddDocumentDescription.SelectedIndex == 0)
            throw new HandledException(1, Constants.REQUIREMENT_ADVERTENCIA_Seleccione_Tipo_Documento);
          if (this.wddDocumentDescription.SelectedValue == "1" && this.txtNumeroRecabante.Text.Trim().Length != 8)
            throw new HandledException(1, Constants.REQUIREMENT_ADVERTENCIA_DNI_8);
          if (this.wddDocumentDescription.SelectedValue == "4" && this.txtNumeroRecabante.Text.Trim().Length != 11)
            throw new HandledException(1, Constants.REQUIREMENT_ADVERTENCIA_RUC_11);
          if (this.wddDocumentDescription.SelectedValue == "4" && this.txtNumeroRecabante.Text.Trim().Substring(0, 1) != "1" && this.txtNumeroRecabante.Text.Trim().Substring(0, 1) != "2")
            throw new HandledException(1, Constants.REQUIREMENT_ADVERTENCIA_RUC_Longitud);
          if (this.wddDocumentDescription.SelectedValue == "4" && !this.ValidateRuc(this.txtNumeroRecabante.Text.Trim()))
            throw new HandledException(1, Constants.REQUIREMENT_ADVERTENCIA_RUC_Formato);
        }
        ...
        {
          foreach (DataRow row in (InternalDataCollectionBase) (this.Session["dtWastageControlOperation"] as DataTable).Rows)
          {
            if (row["v_Status"].ToString() == "SELECCIONADO")
            {
    ...
    private void LoadParameter()
    {
      try
      {
        ...
        this.hdUseCorrespondence.Value = this.Request.QueryString["strUseCorrespondence"].ToString((IFormatProvider) CultureInfo.CurrentCulture);
        ...
        if (result == 1)
        {
          ...
          this.rblPersonTypeCollect.SelectedValue = "1";
      ...
      int pintTotBlank)
    {
      try
      {
        int pintRequirementPlateId = pintRequirementPlateID;
        ...
        switch ((enmOwnerPersonType) Convert.ToInt32(this.hdTipPer.Value, (IFormatProvider) CultureInfo.CurrentCulture))
        {
          ...
          case enmOwnerPersonType.Juridica:
            ...
            nullable3 = new int?(2);
            if (int32_2 == enmTypeCollect.Apoderado)
            {
              nullable2 = new int?(2);
              if (int32_3 == enmPersonTypecollect.Otros)
              {
                str3 = this.txtName.Text;
                nullable1 = new int?(Convert.ToInt32(this.wddDocumentDescription.SelectedValue, (IFormatProvider) CultureInfo.CurrentCulture));
                str4 = this.txtNumeroRecabante.Text;
              }
              else if (int32_3 == enmPersonTypecollect.RepresentanteLegal)
              {
                str3 = this.txtName.Text;
                nullable1 = new int?(Convert.ToInt32(this.wddDocumentDescription.SelectedValue, (IFormatProvider) CultureInfo.CurrentCulture));
                str4 = this.txtNumeroRecabante.Text;
        ...
        new PlateDeliverManagementBL().PlateDeliverDataInsert(new PlateDeliverData()
        {
          ...
          i_RequirementPlateId = new int?(pintRequirementPlateId),
          v_ProductCompositionId = str1,
          i_CollectTypeId = nullable2,
          i_CollectPersonTypeId = nullable3,
          i_RepresentativeCollectTypeId = this.rblPersonTypeCollect.SelectedValue == string.Empty ? new int?() : new int?(Convert.ToInt32(this.rblPersonTypeCollect.SelectedValue, (IFormatProvider) CultureInfo.CurrentCulture)),
          i_InsertUserId = new int?(iSystemUserId),
          d_ConsMTCDate = nullable4,
          d_TachaSUNARPDate = nullable5,
          v_ReferenceNumberConsMTC = str5,
          v_ReferenceNumberTachaSUNARP = str6,
          v_PlateTacha = (string) null
        }, 0);
        new PlateDeliverManagementBL().SearchIdsInWarehouseControlDetailUpdate(pintRequirementPlateId, iSystemUserId);
        this.InterfaceWithWarehouse(pintRequirementPlateID, pintPlateOldType, pintTotBlank);
        this.UpdateOtherProcess(pintRequirementPlateID);
        Message.SetMessage(this.lblMessage, new HandledException(2, "Proceso de Entrega Registrado con éxito."));
    ...
    private void IsValidRequiredDocuments(int pintProcessTypeId, int pintOwnerPersonType)
    {
      try
      {
        string str = this.hdUseCorrespondence.Value.ToString((IFormatProvider) CultureInfo.CurrentCulture);
