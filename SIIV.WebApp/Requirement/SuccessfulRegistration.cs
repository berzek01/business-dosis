...
namespace SIIV.WebApp.Requirement
{
  public class SuccessfulRegistration : Page
  {
    ...
    protected HtmlTableRow trMessageCode;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        string str1 = this.Request.QueryString["RequirementPlateId"].ToString((IFormatProvider) CultureInfo.CurrentCulture);
        ...
        if (this.Session["SendEmail"] != null)
        {
          ...
          if (dataTrazabilityId.Rows.Count > 0)
            this.SendEmailTrazability(dataTrazabilityId);
        }
        double num = Math.Round(cur.Rows[0]["f_PriceTotal"].ToString() != "" ? Convert.ToDouble(cur.Rows[0]["f_PriceTotal"]) : 0.0, 2);
        if (Convert.ToInt32(this.Session["VisaPagoConforme"]) == 1)
        {
          ...
          this.lblPaymentCode.Text = new RequirementQueriesBL().GetPaymentCode(Convert.ToInt32(str1, (IFormatProvider) CultureInfo.CurrentCulture));
        this.lblPrice.Text = "S/ " + num.ToString();
        ...
        {
          this.trPaymentCode.Visible = false;
          this.trMessageCode.Visible = false;
        }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
      ...
      bool boolean = Convert.ToBoolean(string.IsNullOrEmpty(ConfigurationManager.AppSettings["SaveLogEmail"]) ? "false" : (ConfigurationManager.AppSettings["SaveLogEmail"] == "1" ? "true" : "false"));
      if (boolean)
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "Log de Crear Cur - Inicio", enmFileSection.Header);
      try
      {
        this.generateCur(1);
        if (!boolean)
          return;
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "Log de Crear Cur - Fin", enmFileSection.Header);
      }
      catch (HandledException ex)
      {
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "SuccefulRegistration.aspx - Error en el método de Generar Cur - HandledException : " + ex.GetType().Name + " Message: " + ex.Message.ToString() + "; \nStackTrace: " + ex.StackTrace.ToString(), enmFileSection.Content);
        SIIV.Common.Resource.Message.SetMessage(this.lblMessage, ex);
      }
      catch (Exception ex)
      {
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "SuccefulRegistration.aspx - Error en el método de Generar Cur - Exception : " + ex.GetType().Name + " Message: " + ex.Message.ToString() + "; \nStackTrace: " + ex.StackTrace.ToString(), enmFileSection.Content);
        SIIV.Common.Resource.Message.SetMessage(this.lblMessage, new HandledException(-100, ex));
      }
    }
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
      try
      {
        if (this.Request.QueryString["mre"] == "0")
        ...
        else if (this.Request.QueryString["mre"] == "1")
          this.Response.Redirect("~/Requirement/RegisterRequirementMRE.aspx");
        else
    ...
    {
      this.Response.Redirect("~/Delivery/BookQuery.aspx?Plate=" + this.lblPlateRequirement.Text);
    }
    ...
    protected void btnSend_Click(object sender, EventArgs e)
    {
      ...
      if (boolean1)
      {
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "SuccefulRegistration.aspx -  Log de Envio de Correo", enmFileSection.Header);
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "SuccefulRegistration.aspx -  Class Mail", enmFileSection.Content);
      }
      try
      {
        ...
        if (boolean1)
          SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "SuccefulRegistration.aspx - Inicio de proceso - pstrSMTPUserName: " + userName + "; Asunto: " + str2, enmFileSection.Content);
        dataTable2.Dispose();
        ...
        if (!boolean1)
          return;
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "SuccefulRegistration.aspx - Fin de proceso ", enmFileSection.Content);
      }
      catch (HandledException ex)
      {
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "SuccefulRegistration.aspx - Error en el método de Envío de Correo - HandledException : " + ex.GetType().Name + " Message: " + ex.Message.ToString() + "; \nStackTrace: " + ex.StackTrace.ToString(), enmFileSection.Content);
        SIIV.Common.Resource.Message.SetMessage(this.lblMessage, ex);
      }
      catch (Exception ex)
      {
        SIIV.Common.Resource.Utilities.Logging.WriteFileLog(strFile, "SuccefulRegistration.aspx - Error en el método de Envío de Correo - Exception : " + ex.GetType().Name + " Message: " + ex.Message.ToString() + "; \nStackTrace: " + ex.StackTrace.ToString(), enmFileSection.Content);
        SIIV.Common.Resource.Message.SetMessage(this.lblMessage, new HandledException(-100, ex));
      }
    }
      ...
      try
      {
        ...
        if (this.Request.QueryString["RequirementId"] != null)
        {
          ...
          using (ReportDocument reportDocument = new ReportDocument())
          {
            ...
            DataTable cur2 = new RequirementQueriesBL().GenerateCUR(int32);
            if (cur2 != null)
            {
              if (cur2.Rows.Count > 1)
                cur2.Rows.RemoveAt(1);
              cur2.Columns.Add(new DataColumn()
              {
                ColumnName = "ImageBarPlate",
                DataType = typeof (byte[])
              });
              byte[] numArray1 = this.ImagenBarCode(cur2.Rows[0]["v_PlateNew"].ToString());
              cur2.Rows[0]["ImageBarPlate"] = (object) numArray1;
              cur2.Columns.Add(new DataColumn()
              {
                ColumnName = "Requisite",
                DataType = typeof (string)
              });
              string requisitebyRequirement = new RequirementQueriesBL().GetRequisitebyRequirement(int32, Convert.ToInt32(cur2.Rows[0]["i_ProcessTypeId"], (IFormatProvider) CultureInfo.CurrentCulture));
              cur2.Rows[0]["Requisite"] = (object) requisitebyRequirement;
              cur2.Columns.Add(new DataColumn()
              {
                ColumnName = "ImageBarCode",
                DataType = typeof (byte[])
              });
              byte[] numArray2 = this.ImagenBarCode(cur2.Rows[0]["v_PaymentCode"].ToString());
              cur2.Rows[0]["ImageBarCode"] = (object) numArray2;
    ...
    public void SendEmailTrazability(DataTable dt)
    {
      try
      {
        ...
        string str3 = "1";
        int int32_1 = Convert.ToInt32(dt.Rows[0]["i_EmailNotificationId"]);
        string strPlaca = dt.Rows[0]["v_PlateNew"].ToString();
        string strSolicitud = dt.Rows[0]["i_RequirementPlateId"].ToString();
        string strTipoSolicitud = Convert.ToInt32(dt.Rows[0]["i_RequirementPlateTypeId"]) == 9 ? "Solicitud Masiva" : "Solicitud Regular";
        string strFechaSolicitud = dt.Rows[0]["fecha_solicitud"].ToString();
        string strPuntoEntrega = dt.Rows[0]["v_DeliveryPointId"].ToString();
        string strCliente = dt.Rows[0]["v_CompleteName"].ToString();
        string strCodigoPago = dt.Rows[0]["codigo_Pago"].ToString();
        string strTotal = dt.Rows[0]["precio"].ToString();
        string str4 = dt.Rows[0]["v_Email"].ToString();
        int int32_2 = Convert.ToInt32(dt.Rows[0]["i_StatusRequirement"]);
        ...
        switch (int32_2)
        {
          ...
          case 1:
            ...
            empty3 = dt.Rows[0]["banco"].ToString();
            empty4 = dt.Rows[0]["fecha_Pago"].ToString();
            break;
        }
