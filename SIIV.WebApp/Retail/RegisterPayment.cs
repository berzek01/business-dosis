...
namespace SIIV.WebApp.Retail
{
  public class RegisterPayment : Page
  {
    ...
    protected Label LblCashRegCode3;
    ...
    protected void MoveNext(object sender, EventArgs e)
    {
      try
      {
        ...
        this.lblMessage.Visible = false;
        if (this.Session["v_CashRegCode"] != null || this.Session["i_CashRegId"] != null)
          this.LblCashRegCode3.Text = this.Session["v_CashRegCode"].ToString();
      }
      catch (HandledException ex)
      {
        Message.SetMessage(this.lblMessage, ex);
      ...
      try
      {
        ...
        if (new RequirementManagementBL().RegisterEBillingData(Convert.ToInt32(this.Session["i_CashRegId"].ToString()), Convert.ToInt32(this.ViewState["intRequirementID"]), 1, 1, Convert.ToInt32(systemUser.i_SystemUserId)))
        {
          ...
          if (this.rbTypePayment.SelectedIndex == 0)
          {
            ...
            flag = new RequirementManagementBL().EBillingStatusProcess(Convert.ToInt32(this.ViewState["intRequirementID"]), 0, Convert.ToDouble(this.TxtPayTotal.Text.Trim()));
          if (flag)
          {
          ...
          {
            Message.SetMessage(this.lblMessage2, new HandledException(0, "Error en Conexión"));
    ...
    protected void Timer1_Tick(object sender, EventArgs e)
    {
      ...
      try
      {
        if (actionProceesEbilling.Rows[0][0].ToString() == ConfigurationManager.AppSettings["EBillingPayStatus"].ToString() && actionProceesEbilling.Rows[0][1].ToString() == "2")
        {
          ...
          this.Session["Tipo"] = (object) "2";
          this.Response.Redirect(string.Format("~/Retail/SuccessfulRegistration.aspx?id={0}", (object) Convert.ToInt32((this.Session["ListRequirements"] as DataTable).Rows[0]["i_RequirementId"].ToString())), false);
        }
        if (actionProceesEbilling.Rows[0][1].ToString() != "0" && actionProceesEbilling.Rows[0][1].ToString() != "2")
        {
          ...
          Message.SetMessage(this.lblMessage2, new HandledException(0, actionProceesEbilling.Rows[0][2].ToString()));
        }
        ...
        this.ImgLoading2.Visible = false;
        this.rbTypePayment.Enabled = true;
        Message.SetMessage(this.lblMessage2, new HandledException(0, "Error en Conexion"));
      }
      catch
      {
        this.HidePopup();
    ...
    protected void BtnValidate_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        string[] strArray2 = this.rdbProofPayment.SelectedIndex != 0 ? new SunatRuc().SunatRudReader(this.TxtDocNumberProofPaper.Text.Trim(), 2) : new SunatRuc().SunatRudReader(this.TxtDocNumberProofPaper.Text.Trim(), 1);
        this.txtBeneficiaryName.Text = strArray2[0];
        this.txtAddress.Text = strArray2[1];
      }
      catch (Exception ex)
      {
